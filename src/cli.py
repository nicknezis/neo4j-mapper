...
@click.option('--quiet', '-q', is_flag=True, help='Enable quiet mode (warnings and errors only)')
...
@click.option('--format', '-f', type=click.Choice(['csv', 'json', 'cypher', 'all']), 
              default='csv', help='Output format')
@click.option('--mapping', '-m', help='Specific mapping to process (default: all mappings)')
@click.option('--dry-run', is_flag=True, help='Validate configuration without processing data')
def transform(config_file: str, output_dir: str, format: str, mapping: str, dry_run: bool):
    ...
    try:
        ...
        # Get databases and mappings
        databases = config_loader.get_databases(config)
        ...
        if mapping:
            ...
            if not mappings:
                click.echo(f"Error: Mapping '{mapping}' not found in configuration", err=True)
                sys.exit(1)
        
        # Process each mapping
        for mapping_config in mappings:
            _process_mapping(mapping_config, databases, output_dir, format)
...
def inspect(database_path: str, table: str):
    """Inspect SQLite database structure."""
    try:
        # Create a temporary database configuration
        db_config = [{'path': database_path, 'alias': 'temp_db'}]
        
        with SQLiteReader(db_config) as reader:
            if table:
                # Inspect specific table
                _inspect_table(reader, 'temp_db', table)
            else:
                # List all tables
                _inspect_database(reader, 'temp_db', database_path)
...
"""
    ...
    try:
        ...
        click.echo("Edit this file according to your SQLite database structure.")
...
def _process_mapping(mapping_config: Dict[str, Any], databases: list, 
    ...
    with SQLiteReader(databases) as reader:
        ...
        if 'joins' in mapping_config and mapping_config['joins']:
            ...
            if len(source_parts) == 2:
                db_alias, table_name = source_parts
            else:
                db_alias = databases[0]['alias']
                table_name = first_node['source']
            
            df = reader.read_table(db_alias, table_name)
    ...
    if format == 'csv' or format == 'all':
        ...
        result = formatter.format_complete_mapping(nodes_data, relationships_data, mapping_name)
        logger.info(f"Generated CSV files: {len(result['nodes']) + len(result['relationships'])} files")
...
def _print_transformation_stats(stats: Dict[str, Any]):
    ...
    click.echo(f"Total Relationships: {totals['relationship_count']} ({totals['relationship_types']} types)")
    
    # Node details
    if stats['nodes']:
        click.echo("\nNode Types:")
        for label, info in stats['nodes'].items():
            click.echo(f"  - {label}: {info['count']} nodes, {info['properties']} properties")
    
    # Relationship details
    if stats['relationships']:
        click.echo("\nRelationship Types:")
        for rel_type, info in stats['relationships'].items():
            click.echo(f"  - {rel_type}: {info['count']} relationships, {info['properties']} properties")


def _inspect_database(reader: SQLiteReader, alias: str, db_path: str):
...
def _inspect_table(reader: SQLiteReader, alias: str, table: str):
