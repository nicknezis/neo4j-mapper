...
class CSVFormatter:
    ...
    def format_nodes(
        ...
        for node_df in nodes_data:
            ...
            # Get node label
            label = (
                node_df.iloc[0]["_label"]
                if "_label" in node_df.columns
                else "UnknownNode"
            )
    ...
    def format_relationships(
        ...
        for rel_df in relationships_data:
            ...
            rel_type = (
                rel_df.iloc[0]["_type"]
                if "_type" in rel_df.columns
                else "UnknownRelationship"
            )
            ...
            self.logger.info(
                f"Wrote {len(csv_df)} {rel_type} relationships to {filepath}"
            )
    ...
    def format_complete_mapping(
        ...
        result["summary"]["total_relationships"] = sum(
            len(df) for df in relationships_data
        )

        # Generate import script
        if node_files or relationship_files:
            script_path = self.generate_import_script(
                node_files, relationship_files, mapping_name
            )
