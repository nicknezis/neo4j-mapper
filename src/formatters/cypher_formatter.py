...
class CypherFormatter:
    ...
    def format_batch_script(
        ...
        for rel_df in relationships_data:
            ...
            for i in range(0, len(rel_df), batch_size):
                ...
                unwind_statement = self._create_batch_relationship_statement(
                    batch_df, rel_type
                )
                cypher_statements.append(unwind_statement)
                cypher_statements.append("")
    ...
    def _create_batch_node_statement(self, batch_df: pd.DataFrame, label: str) -> str:
        ...
        return f"UNWIND {data_str} as row\n" f"CREATE (n:{label})\n" f"SET n = row;"

    def _create_batch_relationship_statement(
        self, batch_df: pd.DataFrame, rel_type: str
    ) -> str:
        """Create batched relationship creation using UNWIND."""
        # Prepare data for UNWIND
        batch_data = []
        for _, row in batch_df.iterrows():
            rel_data = {"_from_id": row.get("_from_id"), "_to_id": row.get("_to_id")}
