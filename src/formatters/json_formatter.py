...
class JSONFormatter:
    ...
    def format_nodes(self, nodes_data: List[pd.DataFrame], 
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump(nodes_json, f, indent=2, default=self._json_serializer, ensure_ascii=False)
    ...
    def format_relationships(self, relationships_data: List[pd.DataFrame],
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump(relationships_json, f, indent=2, default=self._json_serializer, ensure_ascii=False)
        
        self.logger.info(f"Wrote {len(relationships_json)} relationships to {filepath}")
        return str(filepath)
    
    def format_graph_structure(self, nodes_data: List[pd.DataFrame], 
        ...
        graph_data['metadata']['statistics'] = {
            ...
            'node_types': len(set(node.get('_label', 'Unknown') for node in graph_data['nodes'])),
            'relationship_types': len(set(rel.get('_type', 'Unknown') for rel in graph_data['relationships']))
        }
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump(graph_data, f, indent=2, default=self._json_serializer, ensure_ascii=False)
        
        self.logger.info(f"Wrote complete graph structure to {filepath}")
        return str(filepath)
    
    def format_cypher_compatible(self, nodes_data: List[pd.DataFrame], 
                               relationships_data: List[pd.DataFrame],
                               mapping_name: str = "mapping") -> str:
            ...
            for _, row in node_df.iterrows():
                ...
                for col, value in row.items():
                    if not col.startswith('_') and pd.notna(value):
                        node_data['properties'][col] = self._convert_value_for_cypher(value)
        ...
        for rel_df in relationships_data:
            ...
            for _, row in rel_df.iterrows():
                ...
                for col, value in row.items():
                    if not col.startswith('_') and pd.notna(value):
                        rel_data['properties'][col] = self._convert_value_for_cypher(value)
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump(cypher_data, f, indent=2, default=self._json_serializer, ensure_ascii=False)
    ...
    def format_complete_mapping(self, nodes_data: List[pd.DataFrame], 
        ...
        graph_file = self.format_graph_structure(nodes_data, relationships_data, mapping_name)
        files_generated.append(graph_file)
        
        # Cypher-compatible format
        cypher_file = self.format_cypher_compatible(nodes_data, relationships_data, mapping_name)
