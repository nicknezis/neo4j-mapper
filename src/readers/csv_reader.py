...
class CSVReader:
    ...
    def read_csv_chunked(
        ...
        try:
            ...
            for chunk in pd.read_csv(
                config["path"], chunksize=chunk_size, **csv_options
            ):
                chunk_count += 1
        ...
        for chunk in self.read_csv_chunked(alias, chunk_size):
            ...
            if where_clause:
                try:
                    ...
                    logging.warning(
                        f"Could not apply where clause '{where_clause}' to chunk: {e}"
                    )
    ...
    def get_csv_info(self, alias: str) -> Dict[str, Any]:
        ...
        try:
            ...
            estimated_rows = (
                int((file_size / sample_size) * len(sample_df))
                if sample_size > 0
                else len(sample_df)
            )
    ...
    def read_table(
        ...
        if self.dataframes[alias] is None:
            ...
            for chunk in self.read_table_chunked(
                alias, table_name, None, columns, where_clause
            ):
                chunks.append(chunk)
                total_rows += len(chunk)
            ...
            if chunks:
                ...
                logging.info(
                    f"Combined {len(chunks)} chunks into {len(df)} rows from {alias}"
                )
                return df
            else:
            ...
            if where_clause:
                try:
                    ...
                    logging.warning(
                        f"Could not apply where clause '{where_clause}': {e}"
                    )
