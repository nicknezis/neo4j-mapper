...
class DataMapper:
    ...
    def map_node_properties(self, df: pd.DataFrame, node_config: Dict[str, Any]) -> pd.DataFrame:
        ...
        for prop_config in node_config['properties']:
            ...
            if source and '.' in source:
                ...
                if actual_column is None:
                    self.logger.warning(f"Column '{field_name}' not found in DataFrame for node {node_config['label']}")
                    continue
            ...
            try:
                ...
                self.logger.error(f"Error converting field '{field_name}' to type '{field_type}': {e}")
                continue
        
        self.logger.info(f"Mapped {len(property_mappings)} properties for node {node_config['label']}")
        return result_df
    
    def map_relationship_properties(self, df: pd.DataFrame, 
                                  rel_config: Dict[str, Any]) -> pd.DataFrame:
        ...
        for prop_config in rel_config['properties']:
            ...
            if field_name not in df.columns:
                self.logger.warning(f"Column '{field_name}' not found in DataFrame for relationship")
                continue
            ...
            try:
                ...
                self.logger.error(f"Error converting relationship property '{field_name}': {e}")
                continue
        
        self.logger.info(f"Mapped {len(property_mappings)} properties for relationship {rel_config['type']}")
        return result_df
    
    def extract_node_data(self, df: pd.DataFrame, node_config: Dict[str, Any]) -> pd.DataFrame:
        ...
        self.logger.info(f"Extracted {len(node_df)} unique nodes for label {node_config['label']}")
        return node_df
    
    def extract_relationship_data(self, df: pd.DataFrame, rel_config: Dict[str, Any],
        ...
        if not from_node_config or not to_node_config:
            raise ValueError(f"Could not find node configurations for relationship {rel_config['type']}")
        ...
        self.logger.info(f"Extracted {len(rel_df)} relationships of type {rel_config['type']}")
        return rel_df
    ...
    def _safe_convert(self, value: Any, converter) -> Any:
        ...
        try:
            ...
            self.logger.warning(f"Could not convert value '{value}' using {converter}: {e}")
            return value
    
    def validate_data_types(self, df: pd.DataFrame, 
                          config_properties: List[Dict[str, Any]]) -> List[str]:
