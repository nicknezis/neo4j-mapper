...
class GraphTransformer:
    ...
    def transform_mapping(self, df: pd.DataFrame, 
                         mapping_config: Dict[str, Any]) -> Tuple[List[pd.DataFrame], List[pd.DataFrame]]:
        ...
        for node_config in mapping_config['nodes']:
            try:
                ...
                self.logger.info(f"Processed node: {node_config['label']} ({len(node_df)} records)")
        ...
        if 'relationships' in mapping_config:
            for rel_config in mapping_config['relationships']:
                try:
                    rel_df = self._transform_relationship(df, rel_config, mapping_config['nodes'])
                    relationships_data.append(rel_df)
                    self.logger.info(f"Processed relationship: {rel_config['type']} ({len(rel_df)} records)")
                except Exception as e:
                    self.logger.error(f"Error processing relationship {rel_config['type']}: {e}")
                    raise
        
        return nodes_data, relationships_data
    
    def _transform_node(self, df: pd.DataFrame, node_config: Dict[str, Any]) -> pd.DataFrame:
        ...
        if 'computed_properties' in node_config:
            node_df = self._add_computed_properties(node_df, node_config['computed_properties'])
        
        return node_df
    
    def _transform_relationship(self, df: pd.DataFrame, rel_config: Dict[str, Any],
                              node_configs: List[Dict[str, Any]]) -> pd.DataFrame:
        ...
        rel_df = self.data_mapper.extract_relationship_data(mapped_df, rel_config, node_configs)
        
        # Add any computed properties
        if 'computed_properties' in rel_config:
            rel_df = self._add_computed_properties(rel_df, rel_config['computed_properties'])
        
        return rel_df
    
    def _add_computed_properties(self, df: pd.DataFrame, 
        ...
        for comp_prop in computed_props:
            ...
            try:
                ...
                if expression.startswith('CONCAT('):
                    ...
                    result_df[prop_name] = self._handle_date_format(result_df, expression)
                elif expression.startswith('UPPER(') or expression.startswith('LOWER('):
                    # Handle case conversion
                    result_df[prop_name] = self._handle_case_conversion(result_df, expression)
                else:
    ...
    def validate_graph_data(self, nodes_data: List[pd.DataFrame], 
                          relationships_data: List[pd.DataFrame]) -> List[str]:
        ...
        for rel_df in relationships_data:
            if '_type' not in rel_df.columns or '_from_id' not in rel_df.columns or '_to_id' not in rel_df.columns:
            ...
            if len(rel_df) > 0:
                ...
                if null_from > 0:
                    errors.append(f"Relationship {rel_type} has {null_from} null from_id values")
                if null_to > 0:
                    errors.append(f"Relationship {rel_type} has {null_to} null to_id values")
        
        return errors
    
    def get_transformation_stats(self, nodes_data: List[pd.DataFrame], 
                               relationships_data: List[pd.DataFrame]) -> Dict[str, Any]:
        ...
        for node_df in nodes_data:
            if len(node_df) > 0 and '_label' in node_df.columns:
                ...
                stats['nodes'][label] = {
                    'count': len(node_df),
                    'properties': len([col for col in node_df.columns if not col.startswith('_')])
                }
        ...
        for rel_df in relationships_data:
            if len(rel_df) > 0 and '_type' in rel_df.columns:
                ...
                stats['relationships'][rel_type] = {
                    'count': len(rel_df),
                    'properties': len([col for col in rel_df.columns if not col.startswith('_')])
                }
